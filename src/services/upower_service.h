...
// Will return NULL if `upower_service_global_init` has not been called.
...
// Borrowed until the next "changed" signal. NULL while unavailable.
...
// UpDevice.
const char *upower_device_map_icon_name(UpDevice *device);

// Whether a valid percentage is available for the primary device.
gboolean upower_service_primary_has_percentage(UPowerService *self);
