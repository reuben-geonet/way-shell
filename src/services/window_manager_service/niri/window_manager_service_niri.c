...
static void wm_service_niri_dispose(GObject *object) {
    WMServiceNiri *self = WM_SERVICE_NIRI(object);
    g_clear_pointer(&self->rust, way_shell_wm_free);
    G_OBJECT_CLASS(wm_service_niri_parent_class)->dispose(object);
}

static void wm_service_niri_class_init(WMServiceNiriClass *klass) {
    G_OBJECT_CLASS(klass)->dispose = wm_service_niri_dispose;
...
static void wm_service_niri_init(WMServiceNiri *self) {
    self->rust = way_shell_wm_new_niri(on_workspaces, on_outputs, self);
}
static GPtrArray *get_workspaces(WindowManager *wm) {
    return way_shell_wm_workspaces(WM_SERVICE_NIRI(wm->private)->rust);
}
static GPtrArray *get_outputs(WindowManager *wm) {
    return way_shell_wm_outputs(WM_SERVICE_NIRI(wm->private)->rust);
}
static int focus_workspace(WindowManager *wm, WMWorkspace *workspace) {
    if (!workspace) return -1;
    return way_shell_wm_workspace_action(WM_SERVICE_NIRI(wm->private)->rust,
...
static int rename_workspace(WindowManager *wm, const char *name) {
    return way_shell_wm_named_action(WM_SERVICE_NIRI(wm->private)->rust, 0, name);
}
static int move_workspace(WindowManager *wm, WMOutput *output) {
    if (!output) return -1;
    return way_shell_wm_named_action(WM_SERVICE_NIRI(wm->private)->rust, 1, output->name);
}
...
WindowManager *wm_service_niri_window_manager_init(void) {
