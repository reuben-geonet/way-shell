...
// Removed with the C brightness consumer. Preserve its synchronous return
// contract while driving the existing GLib context to receive the real reply.
fn wait_brightness(
    start: impl FnOnce(Box<dyn FnOnce(Result<(), glib::Error>)>) -> gio::Cancellable,
    timeout: std::time::Duration,
) -> Result<(), glib::Error> {
    use gio::prelude::*;
    let result = std::rc::Rc::new(RefCell::new(None));
    let output = result.clone();
    let cancel = start(Box::new(move |reply| {
        output.replace(Some(reply));
    }));
    let deadline = std::time::Instant::now() + timeout;
    glib::MainContext::ref_thread_default().block_on(async {
        loop {
            if let Some(reply) = result.borrow_mut().take() {
                return reply;
            }
...
/// # Safety
/// Inputs are borrowed live C objects and NUL-terminated strings for this call.
/// Temporary compatibility for C brightness; new callers use the async API.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn logind_service_session_set_brightness(
    pointer: *mut glib::gobject_ffi::GObject,
    subsystem: *const c_char,
    name: *const c_char,
    brightness: u32,
    ...
    catch_unwind(AssertUnwindSafe(|| {
        ...
        let (Ok(subsystem), Ok(name)) = (
            unsafe { CStr::from_ptr(subsystem) }.to_str(),
            unsafe { CStr::from_ptr(name) }.to_str(),
        ) else {
            return -1;
        };
        let result = wait_brightness(
            |done| service.set_brightness(subsystem, name, brightness, done),
...
mod tests {
    use super::*;
    #[test]
    fn legacy_brightness_waits_for_failure_and_cancels_deadline() {
        use gio::prelude::*;
        ...
        context
            .with_thread_default(|| {
                ...
                assert!(
                    result
                        .unwrap_err()
                        .matches(gio::IOErrorEnum::PermissionDenied)
                );
                let cancel = gio::Cancellable::new();
                let result =
                    wait_brightness(|_| cancel.clone(), std::time::Duration::from_millis(2));
                assert!(result.unwrap_err().matches(gio::IOErrorEnum::TimedOut));
